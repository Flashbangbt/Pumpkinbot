...
export const registerCmdGroupDelete = async (client: Client) => { // Change to accept Client type
  const commands = [
  ...
  try {
    ...
    await client.application?.commands.set(commands); // Access the commands directly
