...
export async function handleZoneEmote(
  ...
  if (zoneSize !== "unknown") {
    ...
    if (playerData) {
      ...
      if (typeof playerTeam === 'string' && playerData.team !== playerTeam) {
      ...
      const teamId = (typeof playerTeam === 'string' ? playerTeam : '').match(/Team (\d+)/)?.[1];  // Ensure playerTeam is a string

      if (teamId) {
        const teamInfoResponse = await rce.servers.command(serverId, `teaminfo "#${teamId}"`);

        const leader = (typeof teamInfoResponse === 'string' ? teamInfoResponse : '').match(/(.*)\s\(\w+\)$/)?.[1];  // Ensure teamInfoResponse is a string
        const teamMembers = (typeof teamInfoResponse === 'string' ? teamInfoResponse : '').split("\n").map(line => line.trim());

        // Update player data in the local DB
        await localDB.addOrUpdatePlayer(playerName, typeof playerTeam === 'string' ? playerTeam : '', zoneSize, position);
